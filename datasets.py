...
class ImageNet2012Dataset(Dataset):
    ...
    def __init__(self, file_folder, mode="train", transform=None):
        ...
        if mode == "train":
            self.list_file = os.path.join(self.file_folder, "Annotations", "CLS-LOC", "train.txt")
        else:
            self.list_file = os.path.join(self.file_folder,  "Annotations", "CLS-LOC", "val.txt")

        with open(self.list_file, 'r') as infile:
            for line in infile:
                img_path = line.strip().split()[0]
                img_label = int(line.strip().split()[1])
                self.img_path_list.append(os.path.join(self.file_folder, "Data", "CLS-LOC", self.mode, img_path))
